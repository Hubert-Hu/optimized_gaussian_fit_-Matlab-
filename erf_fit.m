function [a_fit,gauss]=erf_fit(X,Y)
...
a21 = min(X) + (max(X)-min(X))/4;
a22 = min(X) + (max(X)-min(X))/4 * 2;
a23 = min(X) +  (max(X)-min(X))/4 * 3;
%a0 = [a1,a2,a3];
a0 = [a1,a2,a3,a4];
%a0 = [a1,a21,a3,a1,a22,a3,a4];
%a0 = [a1, a21, a3, a4, a1, a22,a3,a1,a23,a3];
...
fit_error = @(param) sum((Y - gauss(param, X)).^2)/sum((Y - mean(Y)).^2);
